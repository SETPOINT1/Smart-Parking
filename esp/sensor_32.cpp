...
void reconnect() {
  while (!client.connected()) {
    ...
    String clientId = "ParkingSensor32-" + String(random(0xffff), HEX);
...
void loop() {
  ...
  if (distance > 4 && distance < threshold && !isCarParked) {
    delay(500);
    ...
    if (checkDist > 4 && checkDist < threshold) {
      ...
      client.publish("parking/slot2", "OCCUPIED");
      isCarParked = true;
    }
  }

  
  else if (distance > threshold && isCarParked) {
    delay(500);
    ...
    if (checkDist > threshold) {
      ...
      digitalWrite(redLed, LOW);
      digitalWrite(greenLed, HIGH);
      client.publish("parking/slot2", "FREE");
      isCarParked = false;
    }
  }
